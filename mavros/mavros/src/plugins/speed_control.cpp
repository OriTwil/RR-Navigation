...
private:
	...
	void handle_speed_control(const mavlink::mavlink_message_t *msg, mavlink::common::msg::POSTURE &posture_state)
	{
		...
		posture_state_msg->w_z = posture_state.w_z;
		posture_state_msg->xangle = posture_state.xangle;
		posture_state_msg->yangle = posture_state.yangle;
		posture_state_msg->zangle = posture_state.zangle;
		...
		if(last_point!=posture_state.point&&pass_time.toSec()>1)
		{
			...
			ROS_INFO("point = %d",zone_msg->point);
		}
		else
		{
			zone_msg->point = -1;
	...
	// void handle_speed_control(const mavlink::mavlink_message_t *msg, mavlink::common::msg:: &speed_control)
	// {
